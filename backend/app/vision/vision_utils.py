"""
기숙사 퇴사 점검 — 이미지 정합 및 차이 검출 유틸리티.

기존 루트의 vision_utils.py 를 backend/app/vision/ 패키지로 이식한 파일입니다.
알고리즘·파라미터는 원본을 그대로 보존합니다.

변경 내역 (원본 대비):
  - detect_difference 반환 타입 주석 수정:
      tuple[np.ndarray, int]  →  list[tuple[int, int, int, int]]
    (실제 반환값에 맞게 주석만 정리, 동작은 동일)
  - 모듈 독스트링 보강
  - _MIN_CONTOUR_AREA 주석에 "미사용" 표기 추가

TODO (성능 개선은 Phase 4 이후에 진행):
  - ORB nfeatures 파라미터 환경변수 외부화
  - SSIM win_size / Otsu 최소 threshold 튜닝
  - 멀티스케일 차이 검출 검토
  - GPU 가속 (cv2.cuda) 검토
"""
...
# ORB 매칭·RANSAC 에 필요한 최소 좋은 매칭 개수 (너무 적으면 호모그래피가 불안정합니다)
_MIN_GOOD_MATCHES = 15
# 노이즈로 간주하여 무시할 최소 윤곽선 면적(픽셀) — 현재 미사용 (detect_difference 내 지역 상수 사용)
...
def align_images(ref_img: np.ndarray, curr_img: np.ndarray) -> np.ndarray:
    ...
    Notes
    -----
    - Lowe 비율 테스트(threshold 0.75)로 불안정한 매칭을 줄입니다.
...
def detect_difference(
    ...
    """
    정합된 두 이미지 간의 SSIM 차이 영역을 검출하여 bounding box 목록을 반환합니다.
    ...
    -------
    list[tuple[int, int, int, int]]
        차이 영역의 bounding box 목록. 각 항목은 (x, y, width, height) 픽셀 좌표입니다.
        이슈가 없으면 빈 리스트를 반환합니다.
    ...
    -----
    파이프라인 순서:
      GaussianBlur → SSIM → Otsu threshold (min 100) → valid mask →
      Morphological open/close → findContours → 면적 필터 → boundingRect
    """
    MIN_AREA = 3000
    ...
    # warpPerspective 로 생긴 검은 테두리를 valid_mask 로 제거
